...
Tinypass.prototype.encrypt = function(text) {
  ...
  for (var i = 0; i <= textAsBytes.length; i += blockSize) {
    plainTextByteBlock = textAsBytes.slice(i, i + blockSize);

    padding = new Buffer(blockSize - plainTextByteBlock.length);
    padding.fill('\0');
...
function ensafe(data) {
    return _.trimRight(data.toString('base64').replace('+/', '-_'), '=');
}
